...
import { parseListingDescription } from '../../../services/aiService';
...
export const useListingForm = (user: User | null, listings: Listing[], activeBookings: Booking[], editingListing: Listing | null, refreshData: () => void, setView: (view: any) => void, onCreateListing?: (l: Listing) => void, onUpdateListing?: (l: Listing) => void) => {
    ...
    const handleAiAutoFill = async () => {
        ...
        try {
            const extractedData = await parseListingDescription(currentPrompt);
            
            if (Object.keys(extractedData).length === 0) {
                toast.showToast({ message: 'Could not generate listing details. Please try again.', type: 'error' });
                return;
            }
            ...
            toast.showToast({ message: "✨ Claude AI has filled in your listing details!", type: "success" });
        } catch (error) {
            console.error('AI Auto-fill error:', error);
            toast.showToast({ message: "Failed to generate listing details.", type: "error" });
        } finally {
