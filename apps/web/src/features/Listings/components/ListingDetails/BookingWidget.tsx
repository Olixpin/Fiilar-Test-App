    ...
    <div className="lg:col-span-1">
      ...
      <div className="hidden lg:block sticky top-24 glass-card rounded-2xl p-6 shadow-xl shadow-black/5">
        ...
        <div className="space-y-4 mb-6">
          ...
          {/* Guest Selection - NEW MODEL v1.2 - Separate fields for clarity */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Guests</label>
            {(() => {
              // Use new model with fallback to legacy (maxGuests already computed at component level)
              ...
              return (
                <div className="space-y-3">
                  {/* Base Guests - Always visible */}
                  <div className="p-3 bg-gray-50 rounded-xl">
                    <div className="flex items-center justify-between mb-2">
                      <div>
                        <span className="text-sm font-medium text-gray-900">Standard guests</span>
                        <p className="text-xs text-gray-500">Included in base price</p>
                      </div>
                    ...
                    <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden bg-white">
                      <button
                        ...
                        className="px-3 py-2 hover:bg-gray-50 border-r border-gray-200 disabled:opacity-40 disabled:cursor-not-allowed"
                      >
                        <Minus size={14} />
                      </button>
                      ...
                      <button
                        ...
                        className="px-3 py-2 hover:bg-gray-50 border-l border-gray-200 disabled:opacity-40 disabled:cursor-not-allowed"
                      >
                        <Plus size={14} />
                      </button>
                    </div>
                  ...
                  {/* Extra Guests Section - Only if host allows */}
                  {allowExtraGuests && extraGuestLimit > 0 && extraGuestFee > 0 && (
                    <div className={`p-3 rounded-xl border-2 transition-all ${wantsExtra ? 'bg-emerald-50 border-emerald-200' : 'bg-white border-gray-200'}`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <button
                            type="button"
                            onClick={handleExtraToggle}
                            className={`relative w-10 h-5 rounded-full transition-colors ${wantsExtra ? 'bg-emerald-500' : 'bg-gray-300'}`}
                            role="switch"
                            aria-checked={wantsExtra}
                            title="Toggle extra guests"
                          >
                            <span className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${wantsExtra ? 'translate-x-5' : ''}`} />
                          </button>
                          <div>
                            <span className="text-sm font-medium text-gray-900">Need extra guests?</span>
                            <p className="text-xs text-gray-500">
                              +{formatCurrency(extraGuestFee)} each (up to {extraGuestLimit})
                            </p>
                          </div>
                        </div>
                      </div>
                      
                      {/* Extra guest counter - only when toggled on */}
                      {wantsExtra && (
                        <div className="mt-3 pt-3 border-t border-emerald-200">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden bg-white">
                              <button
                                ...
                                className="px-2 py-1 hover:bg-gray-50 border-r border-gray-200 disabled:opacity-40"
                              >
                                <Minus size={12} />
                              </button>
                              <div className="w-10 text-center py-1 font-semibold text-gray-900 text-sm">
                                {extraGuestCount}
                              </div>
                              <button
                                type="button"
                                ...
                                className="px-2 py-1 hover:bg-gray-50 border-l border-gray-200 disabled:opacity-40"
                              >
                                <Plus size={12} />
                              </button>
                            </div>
                          ...
                          {extraGuestCount > 0 && (
                            <p className="text-xs text-emerald-600 mt-2 font-medium">
                              Extra guest fee: {formatCurrency(extraGuestCount * extraGuestFee)}
                            </p>
                          )}
                        </div>
                      )}
      ...
      <div className="space-y-4">
        <div className="border-t border-gray-100 pt-4 space-y-2">
          {/* Show extra guest fees separately if applicable */}
          {(() => {
            const maxGuests = listing.maxGuests ?? listing.capacity ?? 1;
            const allowExtraGuests = listing.allowExtraGuests ?? false;
            const extraGuestFee = listing.extraGuestFee ?? listing.pricePerExtraGuest ?? 0;
            const extraGuestsSelected = allowExtraGuests ? Math.max(0, guestCount - maxGuests) : 0;
            const hasExtraGuests = extraGuestsSelected > 0 && extraGuestFee > 0;
                ...
                {hasExtraGuests && (
                  <>
                    <div className="flex justify-between text-sm text-gray-600">
                      <span>Base ({maxGuests} guest{maxGuests > 1 ? 's' : ''}){isRecurring && ` × ${recurrenceCount}`}</span>
                      <span>{formatCurrency((fees as any).rentalSubtotal || (fees.subtotal - extraGuestsSelected * extraGuestFee * (isRecurring ? recurrenceCount : 1)))}</span>
                    </div>
                    <div className="flex justify-between text-sm text-brand-600">
                      <span>+ {extraGuestsSelected} extra guest{extraGuestsSelected > 1 ? 's' : ''} × {formatCurrency(extraGuestFee)}{isRecurring && ` × ${recurrenceCount}`}</span>
                      <span>{formatCurrency((fees as any).extraGuestTotal || (extraGuestsSelected * extraGuestFee * (isRecurring ? recurrenceCount : 1)))}</span>
                    </div>
                  </>
                )}
                {!hasExtraGuests && (
