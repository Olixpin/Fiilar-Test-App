                  ...
                  <div className="p-3 bg-gray-50 rounded-xl">
                    <div className="flex items-center justify-between mb-2">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">Included in price</p>
                      </div>
                      <span className="text-xs font-medium text-gray-400 bg-gray-200 px-2 py-0.5 rounded">max {maxGuests}</span>
                    </div>
                    <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden bg-white">
                      <button type="button" onClick={() => updateBaseGuests(baseGuestCount - 1)} disabled={baseGuestCount <= 1} aria-label="Decrease guests" title="Decrease guests" className="px-4 py-2.5 hover:bg-gray-100 disabled:opacity-50"><Minus size={16} /></button>
                      <div className="flex-1 text-center py-2 font-bold text-gray-900">{baseGuestCount}</div>
                      <button type="button" onClick={() => updateBaseGuests(baseGuestCount + 1)} disabled={baseGuestCount >= maxGuests} aria-label="Increase guests" title="Increase guests" className="px-4 py-2.5 hover:bg-gray-100 disabled:opacity-50"><Plus size={16} /></button>
                    </div>
                  </div>
                    ...
                    <div className={`p-3 rounded-xl border-2 transition-all ${wantsExtra ? 'bg-emerald-50 border-emerald-200' : 'bg-white border-gray-200'}`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <span className="text-sm font-medium text-gray-900">Need more?</span>
                            <p className="text-xs text-gray-500">+{formatCurrency(extraGuestFee)}/guest</p>
                          </div>
                        </div>
                        <span className="text-xs text-gray-400">up to {extraGuestLimit}</span>
                      </div>
                      ...
                      {wantsExtra && (
                        <div className="mt-3 pt-3 border-t border-emerald-200">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden bg-white">
                              <button type="button" onClick={() => updateExtraGuests(extraGuestCount - 1)} disabled={extraGuestCount <= 0} aria-label="Decrease extra guests" title="Decrease extra guests" className="px-3 py-1.5 hover:bg-gray-50 disabled:opacity-40"><Minus size={14} /></button>
                              <div className="w-10 text-center py-1.5 font-bold text-gray-900 text-sm">{extraGuestCount}</div>
                              <button type="button" onClick={() => updateExtraGuests(extraGuestCount + 1)} disabled={extraGuestCount >= extraGuestLimit} aria-label="Increase extra guests" title="Increase extra guests" className="px-3 py-1.5 hover:bg-gray-50 disabled:opacity-40"><Plus size={14} /></button>
                            </div>
                          </div>
                          {extraGuestCount > 0 && (
                            <p className="text-xs text-emerald-600 font-medium mt-2">+{formatCurrency(extraGuestFee * extraGuestCount)} extra</p>
                          )}
                        </div>
                      )}
                    </div>
