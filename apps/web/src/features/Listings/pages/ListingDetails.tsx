...
import { ArrowLeft, MapPin, Star, Calendar as CalendarIcon, AlertCircle, User as UserIcon, ShieldCheck, UploadCloud, X, Loader2, UserCheck, Repeat, Info, ChevronLeft, ChevronRight, CheckCircle, Ban, Users, Plus, Minus, PackagePlus, Grid, Share, Heart, Wallet, CreditCard, MessageSquare, Wifi, Wind, Utensils, Car, Dumbbell, Waves, Briefcase, Tv, Shirt, Scissors, Zap, Coffee, Music, Video, Shield, Sun } from 'lucide-react';
...
const ListingDetails: React.FC<ListingDetailsProps> = ({ listing, user, onBack, onBook, onVerify, onLogin, onRefreshUser }) => {
  ...
  const stateRef = React.useRef({
    ...
    selectedHours,
    selectedDays,
    isHourly: false, // Initial default
    isBookingSubmitted,
    fees: { total: 0, serviceFee: 0, cautionFee: 0, subtotal: 0 } // Initial dummy
  });
  ...
  // Auto-save on unmount (Browser Back / Tab Close)
  useEffect(() => {
    return () => {
  ...
  const saveDraftBooking = (silent = false) => {
    ...
    const reservedBooking: Booking = {
      ...
      status: 'Reserved',
      createdAt: new Date().toISOString(), // Add timestamp for "Saved Xh ago" display
      guestCount: guestCount,
      selectedAddOns: selectedAddOns,
      paymentStatus: undefined
    };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-4 sm:py-8 pb-24 lg:pb-8 animate-in slide-in-from-right duration-300">
      ...
      <div className="flex items-center justify-between mb-4">
        <button
          onClick={() => {
            // Auto-save handled by unmount effect now
            onBack();
          }}
        ...
        <div className="lg:col-span-2 space-y-6 lg:space-y-8">
          ...
          <div className="relative rounded-xl lg:rounded-2xl overflow-hidden h-[300px] sm:h-[400px] group cursor-pointer" onClick={() => openGallery(0)}>
            ...
            {(() => {
              ...
              // 4 Images: 2x2 Grid (or One Large + 3 Small if preferred, but 2x2 is cleaner for 4)
              // Let's do One Large Left (col-span-2, row-span-2) and a column of 3? No, that's for 4 slots.
              // Let's do a 2x2 Grid.
              if (count === 4) {
                return (
            ...
            <div className="space-y-4">
              ...
              {user && (
                <button
                  ...
                  className={`
                    w-full py-3 rounded-xl font-semibold transition border-2 flex items-center justify-center gap-2
                    ${(isHourly && hostOpenHours.length === 0)
      ...
      {/* Success Modal with Handshake Code */}
      {showSuccessModal && confirmedBookings.length > 0 && (
          ...
          <div className="bg-white rounded-2xl max-w-4xl w-full shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[90vh] animate-in zoom-in-95 duration-200">
            ...
            <div className="md:w-2/3 p-8 overflow-y-auto relative">
              <button
                type="button"
                onClick={() => {
                  // Auto-save handled by unmount effect
                  setShowConfirmModal(false);
                }}
