                ...
                <div className="hidden lg:block w-96 flex-shrink-0">
                    <div className="sticky top-24 space-y-4">
                        ...
                        {view === 'create' && (newListing.title || newListing.description) && (
                            <div className={`rounded-lg px-3 py-2 text-xs flex items-center gap-2 transition-all ${
                            ...
                            <div className="p-4">
                                ...
                                <div className="border-t border-gray-100 pt-3 space-y-2">
                                    <div className={`flex items-center gap-2 text-xs ${
                                        newListing.proofOfAddress ? 'text-green-700' : 'text-orange-600'
                                    }`}>
                                        {newListing.proofOfAddress ? (
                                ...
                                {[
                                    ...
                                    <button
                                        ...
                                        className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium transition-colors ${
                                            step === item.step ? 'bg-brand-50 text-brand-700' : 'text-gray-600 hover:bg-gray-50'
                                        }`}
                                    >
                                        <item.icon size={14} />
                                        {item.label}
            ...
            <div className="min-h-screen lg:ml-64">
                ...
                <div className="lg:hidden bg-white border-b border-gray-200 p-4">
                    ...
                    <div className="flex gap-2 mt-3 overflow-x-auto no-scrollbar">
                        <button onClick={() => setView('overview')} className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap ${view === 'overview' ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-700'}`}>Overview</button>
                        <button onClick={() => setView('listings')} className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap ${view === 'listings' ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-700'}`}>Listings</button>
                        <button onClick={() => setView('bookings')} className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap ${view === 'bookings' ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-700'}`}>Bookings</button>
                        <button onClick={() => setView('earnings')} className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap ${view === 'earnings' ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-700'}`}>Earnings</button>
                        <button onClick={() => setView('messages')} className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap ${view === 'messages' ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-700'}`}>Messages</button>
                    </div>
                </div>
